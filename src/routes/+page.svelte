...
<style>
  ...
  /* The last column (54px) is a reserved gutter for the map's own zoom / view-mode buttons. Those are
     NOT in this grid — they live in `Map.svelte`, absolutely positioned bottom-right of the unzoomed
     map layer, which sits *below* the chrome. The column exists purely to keep chrome out of the
     strip they occupy, which is why `.zone-map-controls` is an empty placeholder.
     On a phone the gutter has to be reserved for BOTH content rows, not just the bottom one. The
     button stack is ~176px tall and grows up from the bottom of the map layer, so on a screen this
     short it reaches well into the row above — where the side dock spans into the gutter and covers
     it. That is how the 2D/3D button ends up hidden behind the battery widget in landscape.
     Mobile only: a desktop window is tall enough that the stack never leaves the bottom row, and
     taking the column away from the side dock there would cost its widgets 54px for nothing. */
  .app.mobile {
    grid-template-areas:
      "toolbar      toolbar      toolbar      toolbar"
